    ...
    def action_cost(self, s, a, s1):
        if a.startswith('mover'):
            if (s.garcom_pos, s1.garcom_pos) in self.distancias:
                return self.distancias[(s.garcom_pos, s1.garcom_pos)]
            else:
                return self.distancias[(s1.garcom_pos, s.garcom_pos)]
        else:
            return 1
...
print("=== A* ===")
inicio = time.time()
resultado, nos = astar_search(selecionado, h = selecionado.h3)
fim = time.time()
print(f"Estado final: {resultado.state}")
print(f"Nós explorados: {nos}")
print("Ações:", path_actions(resultado))
