...
class CafeteriaProblem(Problem):
    ...
    def action_cost(self, s, a, s1):
        if a.startswith('mover'):
            if (s.garcom_pos, s1.garcom_pos) in self.distancias:
                return self.distancias[(s.garcom_pos, s1.garcom_pos)] / 2
            else:
                return self.distancias[(s1.garcom_pos, s.garcom_pos)] / 2

        elif a == 'limpar':
            return self.tamanho_mesas[s1.garcom_pos] * 2
            
        elif a == 'preparar_bebida':
            if s1.bebidas_prontas[0] != s.bebidas_prontas[0]:
                return 3
            elif s1.bebidas_prontas[1] != s.bebidas_prontas[1]:
                return 5
    ...
    def h1(self, node):
        # heuristica basica com custo de bebidas a servir e mesas a limpar
        estado = node.state
        bebidas_restantes = sum(f + q for f, q in estado.bebidas)
        mesas_sujas = sum(estado.limpeza)
        return bebidas_restantes + mesas_sujas
    
    def h2(self,node):
        # + custo de deslocamento do garcom pro bar
        estado = node.state
        bebidas_restantes = sum(f + q for f, q in estado.bebidas)
        mesas_sujas = sum(estado.limpeza)

        custo_distancia = 0
        if bebidas_restantes > 0:
            pos = estado.garcom_pos
            if pos != 'bar':
                custo_distancia += self.distancias[('bar', estado.garcom_pos)]
        return bebidas_restantes + mesas_sujas + custo_distancia
    
    def h3(self,node):
        # + custo de entrega de cada bebida
        estado = node.state
        bebidas_restantes = sum(f + q for f, q in estado.bebidas)
        mesas_sujas = sum(estado.limpeza)
        pos = estado.garcom_pos

        custo_distancia_bar = 0
        if bebidas_restantes > 0:
            if pos != 'bar':
                custo_distancia_bar += self.distancias[('bar', estado.garcom_pos)]

        custo_entrega = 0
        if bebidas_restantes > 0:
            for i, pedido in enumerate(estado.bebidas):
                if pedido != (0,0):
                    n_mesa = 'mesa' + str(i+1)
                    custo_entrega += self.distancias[('bar', n_mesa)]
        return bebidas_restantes + mesas_sujas + custo_distancia_bar + custo_entrega

    def h4(self, node):
        estado = node.state
        
        frias_restantes = sum(f for f, q in estado.bebidas)
        quentes_restantes = sum(q for f, q in estado.bebidas)
        
        custo_preparo = frias_restantes * 3 + quentes_restantes * 5
        
        custo_limpeza = sum(self.tamanho_mesas['mesa' + str(i+1)] * 2 
                            for i, suja in enumerate(estado.limpeza) if suja)
        
        custo_deslocamento = 0
        if frias_restantes + quentes_restantes > 0 and estado.garcom_pos != 'bar':
            custo_deslocamento = self.distancias[('bar', estado.garcom_pos)] / 2
        
        return custo_preparo + custo_limpeza + custo_deslocamento
...
# print("=== A* ===")
# inicio = time.time()
# resultado, nos = astar_search(selecionado, h = selecionado.h4)
# fim = time.time()
# print(f"Estado final: {resultado.state}")
# print(f"Nós explorados: {nos}")
# print("Ações:", path_actions(resultado))
# print(f"Tempo: {fim - inicio:.4f}s")
# print(f"Custo total: {resultado.path_cost}")



# Teste automatizado para A*
heuristicas = {
    'h1': lambda p: p.h,
    'h2': lambda p: p.h2,
    'h3': lambda p: p.h3,
...
print("=" * 45)
print(f"{'A*'} {'':13} {'Nós':>8} {'Custo':>8} {'Tempo':>10}")
print("-" * 45)
...
for nome_p, problema in problemas.items():
    for nome_h, h_func in heuristicas.items():
        ...
        fim = time.time()
        print(f"{nome_p:10} {nome_h:5} {nos:>8} {resultado.path_cost:>8.1f} {fim-inicio:>10.4f}s")
    print()
